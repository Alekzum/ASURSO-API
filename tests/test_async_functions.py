...
async def test_attestation():
    attestation = await functions.attestation.get_attestation_async(client=async_client)
...
async def test_dashboard():
    dashboard = await functions.dashboard.get_dashboard_async(client=async_client)
...
async def test_lessons():
    lessons = await functions.lessons.get_lessons_async(client=async_client)
...
async def test_enum_lessons():
    lessons = await functions.lessons.get_lessons_async(
        client=async_client, start=enums.LessonsPeriod.NEXT_DAY
    )
    assert isinstance(lessons, list) and (
        lessons and isinstance(lessons[0], classes.LessonsDay) or True
    )
