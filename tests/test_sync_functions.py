...
def test_attestation():
    attestation = functions.attestation.get_attestation_sync(client=sync_client)
...
def test_dashboard():
    dashboard = functions.dashboard.get_dashboard_sync(client=sync_client)
    assert isinstance(dashboard, classes.Dashboard)


def test_group_attestation():
    group_attestation = functions.reports.get_group_attestation_sync(client=sync_client)
...
def test_lessons():
    lessons = functions.lessons.get_lessons_sync(client=sync_client)
...
def test_enum_lessons():
    lessons = functions.lessons.get_lessons_sync(
        client=sync_client, start=enums.LessonsPeriod.NEXT_DAY
    )
    assert isinstance(lessons, list) and (
        lessons and isinstance(lessons[0], classes.LessonsDay) or True
    )
